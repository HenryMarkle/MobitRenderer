...
void Light_Page::_update_projection_angle() noexcept {
  ...
  _projection_angle =
      (projection_angle(level->light_angle) * level->light_flatness * 10) +
      Vector2{-300, -300};
}

void Light_Page::on_level_loaded() noexcept { _should_redraw = true; }
void Light_Page::on_level_unloaded() noexcept { _should_redraw = true; }
void Light_Page::on_level_selected() noexcept {
  _should_redraw = true;
  _update_projection_angle();
}
void Light_Page::on_page_selected() noexcept { _should_redraw = true; }
...
void Light_Page::process() {
  ...
  if (!_hovering_on_window) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) &&
        _paint_mode != PAINT_SHADOWS_MODE)
      _paint_mode = PAINT_SHADOWS_MODE;
    else if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT) &&
             _paint_mode != PAINT_LIGHT_MODE)
      _paint_mode = PAINT_LIGHT_MODE;
  }
  ...
  if (IsKeyDown(KEY_W)) {
    _brush_height -= _brush_growth_speed +
                     (_brush_growth_speed * 2 * IsKeyDown(KEY_LEFT_SHIFT));
  ...
  if (IsKeyDown(KEY_D)) {
    _brush_width += _brush_growth_speed +
                    (_brush_growth_speed * 2 * IsKeyDown(KEY_LEFT_SHIFT));
  ...
  if (IsKeyDown(KEY_Q)) {
    _brush_rotation -= _brush_growth_speed +
                       (_brush_growth_speed * 2 * IsKeyDown(KEY_LEFT_SHIFT));
    _brush_growth_speed += _brush_growth_accelaration;
  ...
  if (IsKeyReleased(KEY_W) || IsKeyReleased(KEY_S) || IsKeyReleased(KEY_A) ||
      IsKeyReleased(KEY_D) || IsKeyReleased(KEY_Q) || IsKeyReleased(KEY_E)) {
    _brush_growth_speed = 3;
  }

  if (IsKeyDown(KEY_LEFT_CONTROL)) {
    _custom_rotation = true;
    _brush_rotation = atan2(mouse_pos.y - _brush_pinned_pos.y,
                            mouse_pos.x - _brush_pinned_pos.x) *
                          RAD2DEG +
                      90.0f;

    if (!_click_lock)
      _brush_pinned_pos = mouse_pos;
    _click_lock = true;
  } else {
  ...
  if (level != nullptr) {
    if (IsKeyDown(KEY_T)) {
      if (level->light_flatness > 1)
        level->light_flatness--;
      _update_projection_angle();
    } else if (IsKeyDown(KEY_Y)) {
      if (level->light_flatness < 10)
        level->light_flatness++;
      _update_projection_angle();
    }

    if (IsKeyDown(KEY_G)) {
      if (++level->light_angle == 360)
        level->light_angle = 0;
      _update_projection_angle();
    } else if (IsKeyDown(KEY_H)) {
      if (--level->light_angle == 0)
        level->light_angle = 360;
      _update_projection_angle();
    }
  }
...
void Light_Page::draw() noexcept {
  ...
  if (_should_redraw) {
    ...
    for (auto &prop : level->props) {
      if (prop->tile_def == nullptr && prop->prop_def == nullptr)
        continue;
      ...
      mr::sdraw::draw_prop_preview(prop.get(), ctx->_shaders,
                                   ctx->level_layer_ * 10);
    }
    ...
    BeginShaderMode(bkg_shader);
    {
      ...
      SetShaderValue(bkg_shader, GetShaderLocation(bkg_shader, "alpha"), &alpha,
                     SHADER_UNIFORM_INT);
  ...
  if (ctx->_textures->light_editor.brushes().size() > 0) {
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) ||
      ...
      const auto &texture =
          ctx->_textures->light_editor.brushes()[_brush_index].get();
      auto mouse_pos = GetScreenToWorld2D(GetMousePosition(), camera);
      ...
      DrawTexturePro(
          ...
          Rectangle{_custom_rotation ? _brush_pinned_pos.x : mouse_pos.x + 300,
                    _custom_rotation ? _brush_pinned_pos.y : mouse_pos.y + 300,
                    _brush_width, _brush_height},
          {_half_brush_width, _half_brush_height}, _brush_rotation,
  ...
  const auto &lightmap = level->get_lightmap();
  {
    ...
    int alpha = 120;
    SetShaderValue(shader, GetShaderLocation(shader, "alpha"), &alpha,
                   SHADER_UNIFORM_INT);
  ...
  }
  {
    ...
    DrawTextureV(lightmap.texture, _projection_angle, Color{0, 0, 0, 100});
    EndShaderMode();
  }

  mr::draw::draw_double_frame(-300, -300, level->get_pixel_width() + 300,
                              level->get_pixel_height() + 300);
  ...
  if (ctx->_textures->light_editor.brushes().size() > 0) {
    ...
    DrawTexturePro(
        ...
        Rectangle{0, 0, static_cast<float>(texture.width),
                  static_cast<float>(texture.height)},
        Rectangle{_custom_rotation ? _brush_pinned_pos.x : mouse_pos.x,
                  _custom_rotation ? _brush_pinned_pos.y : mouse_pos.y,
                  _brush_width, _brush_height},
        {_half_brush_width, _half_brush_height}, _brush_rotation,
        _paint_mode == PAINT_SHADOWS_MODE ? Color{255, 0, 0, 110}
                                          : Color{255, 120, 120, 110});
    EndShaderMode();
  }
...
void Light_Page::windows() noexcept {
    ...
    if (ImGui::BeginListBox("##List", ImGui::GetContentRegionAvail())) {
      ...
      for (auto &brush : ctx->_textures->light_editor.brushes()) {
        ...
        draw_list->AddRectFilled(
            cursor_pos,
            ImVec2{cursor_pos.x + available_width,
                   cursor_pos.y + available_width},
        ...
        rlImGuiImageSize(brush.get_ptr(), available_width - 20,
                         available_width - 20);

        if (ImGui::GetMouseClickedCount(ImGuiMouseButton_Left) && hovered) {
          _brush_index = counter;
        }
  ...
  if (options_opened && level != nullptr) {
    ...
    if (ImGui::InputInt("Flatness", &level->light_flatness)) {
      if (level->light_flatness > 10)
        level->light_flatness = 1;
      if (level->light_flatness < 1)
        level->light_flatness = 10;
    }
    ...
    if (ImGui::Button("Reset Angle", button_space))
      level->light_angle = 180;
    if (ImGui::Button("Reset Flatness", button_space))
      level->light_flatness = 1;
  }
...
void Light_Page::f3() const noexcept {
  ...
  f3->print("Mode ");
  if (_paint_mode == PAINT_LIGHT_MODE)
    f3->print("Light", true);
  else
    f3->print("Shadow", true);
...
Light_Page::Light_Page(context *ctx)
    : LevelPage(ctx), _hovering_on_window(false), _should_redraw(true),
      _custom_rotation(false), _click_lock(false), _brush_index(0),
      _brush_width(200), _brush_height(200),
      _half_brush_width(_brush_width / 2),
      _half_brush_height(_brush_height / 2), _brush_growth_speed(3),
      _brush_growth_accelaration(0.1f), _brush_rotation(0), _paint_mode(0),
      _projection_angle(Vector2{0, 0}), _brush_pinned_pos(Vector2{0, 0}) {}
