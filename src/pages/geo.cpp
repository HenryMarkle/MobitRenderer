...
bool Geo_Page::_allowed() const noexcept {
  ...
  if (layer > 0 && _geo_category_index == 2 &&
      (_geo_index == 2 || _geo_index == 3))
    return false;
  if (layer > 0 && _geo_category_index == 3)
    return false;
  return true;
}

void Geo_Page::_place(uint16_t x, uint16_t y, uint16_t z, uint16_t width,
                      uint16_t height, uint16_t depth) noexcept {
  ...
  if (!mtx.is_in_bounds(x, y, z))
    return;

  if ((uint16_t)(x + width) < x || (uint16_t)(y + height) < y ||
      (uint16_t)(z + depth) < z)
    return;
  if (!mtx.is_in_bounds(x + width, y + height, z + depth))
    return;
      ...
      for (uint16_t cx = x; cx < x + width; cx++) {
        for (uint16_t cy = y; cy < y + height; cy++) {
          for (uint16_t cz = z; cz < z + depth; cz++) {
            ...
            if (left && top && left->is_solid() && top->is_solid() &&
                (bottom == nullptr || !bottom->is_solid()) &&
                (right == nullptr || !right->is_solid())) {
              if (cell->is_air())
                cell->type = GeoType::slope_es;
            } else if (top && right && top->is_solid() && right->is_solid() &&
                       (left == nullptr || !left->is_solid()) &&
                       (bottom == nullptr || !bottom->is_solid())) {
              if (cell->is_air())
                cell->type = GeoType::slope_sw;
            } else if (right && bottom && right->is_solid() &&
                       bottom->is_solid() &&
                       (top == nullptr || !top->is_solid()) &&
                       (left == nullptr || !left->is_solid())) {
              if (cell->is_air())
                cell->type = GeoType::slope_nw;
            } else if (bottom && left && bottom->is_slope() &&
                       left->is_solid() &&
                       (right == nullptr || !right->is_solid()) &&
                       (top == nullptr || !top->is_solid())) {
              if (cell->is_air())
                cell->type = GeoType::slope_ne;
            }
          }
        }
      }
      ...
      for (uint16_t cy = y; cy < y + height; cy++) {
        for (uint16_t cz = z; cz < z + depth; cz++) {
          ...
          if (is_geo_type_slope(t)) {
            ...
            if (left && top && left->is_solid() && top->is_solid() &&
                (bottom == nullptr ||
                 !(bottom->is_solid() || bottom->is_slope())) &&
                (right == nullptr ||
                 !(right->is_solid() || right->is_slope()))) {
              cells_to_slopes.push_back(cell);
              cells_to_slopes_types.push_back(GeoType::slope_es);
            } else if (top && right && top->is_solid() && right->is_solid() &&
                       (left == nullptr ||
                        !(left->is_solid() || left->is_slope())) &&
                       (bottom == nullptr ||
                        !(bottom->is_solid() || bottom->is_slope()))) {
              cells_to_slopes.push_back(cell);
  ...
  default: {
    ...
    case 1:
      switch (_geo_index) {
      case 0:
        f = GeoFeature::vertical_pole;
        break;
      case 1:
        f = GeoFeature::horizontal_pole;
        break;
      case 2:
        f = GeoFeature::cracked_terrain;
        break;
      }
      break;
    ...
    case 2:
      switch (_geo_index) {
      case 0:
        f = GeoFeature::bathive;
        break;
      case 1:
        f = GeoFeature::forbid_fly_chains;
        break;
      case 2:
        f = GeoFeature::worm_grass;
        break;
      case 3:
        f = GeoFeature::waterfall;
        break;
      }
      break;
    ...
    case 3:
      switch (_geo_index) {
      case 0:
        f = GeoFeature::shortcut_entrance;
        break;
      case 1:
        f = GeoFeature::shortcut_path;
        break;
      case 2:
        f = GeoFeature::room_entrance;
        break;
      case 3:
        f = GeoFeature::dragon_den;
        break;
      case 4:
        f = GeoFeature::wack_a_mole_hole;
        break;
      case 5:
        f = GeoFeature::scavenger_hole;
        break;
      case 6:
        f = GeoFeature::garbage_worm_hole;
        break;
      }
      break;
    }
...
void Geo_Page::_erase(uint16_t x, uint16_t y, uint16_t z, uint16_t width,
                      uint16_t height, uint16_t depth) noexcept {
  if (width == 0 || height == 0 || depth == 0)
    return;
...
void Geo_Page::f3() const noexcept {
  ...
  if (level != nullptr) {
    ...
    const auto *cell = mtx.get_const_ptr(_mtx_mouse_pos.x, _mtx_mouse_pos.y,
                                         ctx->level_layer_);
    if (cell != nullptr) {
  ...
  if (_edit_mode == EDIT_MODE_PLACE)
    f3->print("Place", GREEN, true);
  else if (_edit_mode == EDIT_MODE_ERASE)
    f3->print("Erase", RED, true);
  else
...
void Geo_Page::on_mtx_pos_changed() noexcept {
  ...
  if (_selection_rect.width == 0)
    _selection_rect.width = 20;
  else
    _selection_rect.width += 20;

  if (_selection_rect.height == 0)
  ...
  if (!_hovering_on_window) {
    ...
    if (_is_selecting) {
      ...
      if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) ||
        ...
        if (_edit_mode == EDIT_MODE_PLACE) {
          _place(_selection_rect.x / 20, _selection_rect.y / 20,
                 ctx->level_layer_, _selection_rect.width / 20,
                 _selection_rect.height / 20);

          bool geos_or_features = _geo_category_index == 0 ||
                                  (_geo_category_index == 1 && _geo_index != 2);
          ...
          _erase(_selection_rect.x / 20, _selection_rect.y / 20,
                 ctx->level_layer_, _selection_rect.width / 20,
                 _selection_rect.height / 20);
      ...
      if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) ||
        ...
        _is_selecting = true;
        _selection_rect = Rectangle{_mtx_mouse_pos.x * 20.0f,
                                    _mtx_mouse_pos.y * 20.0f, 20.0f, 20.0f};
        _selection_origin =
            Vector2{_mtx_mouse_pos.x * 20.0f, _mtx_mouse_pos.y * 20.0f};
      }
    }
  }
...
void Geo_Page::draw() noexcept {
  ...
  if (should_redraw_feature1) {
    ...
    mr::draw::draw_geo_features_layer(mtx, ctx->_textures->geometry_editor, 0,
                                      BLACK);

    mr::draw::draw_geo_cracked(mtx, ctx->_textures->geometry_editor, 0, BLACK);

    mr::draw::draw_geo_entrances(mtx, ctx->_textures->geometry_editor, BLACK);
  ...
  if (should_redraw_feature2) {
    ...
    mr::draw::draw_geo_features_layer(mtx, ctx->_textures->geometry_editor, 1,
                                      BLACK);

    mr::draw::draw_geo_cracked(mtx, ctx->_textures->geometry_editor, 1, BLACK);
  ...
  if (should_redraw_feature3) {
    ...
    mr::draw::draw_geo_features_layer(mtx, ctx->_textures->geometry_editor, 2,
                                      BLACK);

    mr::draw::draw_geo_cracked(mtx, ctx->_textures->geometry_editor, 2, BLACK);
  ...
  if (ctx->get_config()->geometry.grid.visible) {
    mr::draw::draw_nested_grid(width, height, Color{255, 255, 255, 90});
  }
  ...
  DrawRectangleLinesEx(
      Rectangle{
          ...
          (level->get_width() - features_border.right - features_border.left) *
              20.0f,
          (level->get_height() - features_border.bottom - features_border.top) *
              20.0f,
      },
      4, WHITE);
  ...
  if (ctx->get_config()->geometry.ruler.visible) {
    ...
    mr::draw::draw_ruler(
        _mtx_mouse_pos.x, _mtx_mouse_pos.y, level->get_width(),
        level->get_height(),
        Color{255, 255, 255,
              static_cast<uint8_t>(ctx->get_config()->geometry.ruler.opacity)});
  }
  ...
  if (ctx->get_config()->geometry.coordinates.visible) {
    ...
    DrawTextEx(
        ...
        _is_selecting
            ? TextFormat("w%d h%d",
                         static_cast<int>(_selection_rect.width) / 20,
                         static_cast<int>(_selection_rect.height) / 20)
            : TextFormat("x%d y%d", _mtx_mouse_pos.x, _mtx_mouse_pos.y),
        Vector2{(_mtx_mouse_pos.x + 1) * 20.0f, (_mtx_mouse_pos.y - 1) * 20.0f},
        22, 0.03f,
        Color{255, 255, 255,
              static_cast<uint8_t>(
                  ctx->get_config()->geometry.coordinates.opacity)});
  }

  if (_is_selecting) {
    DrawRectangleLinesEx(_selection_rect, 2,
                         _edit_mode == EDIT_MODE_PLACE ? WHITE
                         : EDIT_MODE_ERASE             ? RED
                                                       : WHITE);
  } else if (_is_mouse_in_mtx_bounds) {
    DrawRectangleLinesEx(Rectangle{_mtx_mouse_pos.x * 20.0f - 2,
                                   _mtx_mouse_pos.y * 20.0f - 2, 20.0f + 4,
                                   20.0f + 4},
                         2,
                         _edit_mode == EDIT_MODE_PLACE ? WHITE
                         : EDIT_MODE_ERASE             ? RED
  ...
  if (_is_mouse_in_mtx_bounds) {
    ...
    case 0:
      ...
      case 1:
        DrawTriangle(Vector2{scaled_pos.x, scaled_pos.y},
                     Vector2{scaled_pos.x, scaled_pos.y + size},
                     Vector2{scaled_pos.x + size, scaled_pos.y + size}, color);
        break;

      case 2:
        DrawRectangle(scaled_pos.x, scaled_pos.y, size, size / 2, color);
        break;
      }
      break;
    ...
    case 1:
      switch (_geo_index) {
      case 0:
        DrawRectangle(scaled_pos.x + size / 7 * 3, scaled_pos.y, size / 7, size,
                      color);
        break;

      case 1:
        DrawRectangle(scaled_pos.x, scaled_pos.y + size / 7 * 3, size, size / 7,
                      color);
        break;
      ...
      case 2:
        DrawTexturePro(
            textures.ui_crack(),
            Rectangle{0, 0, static_cast<float>(textures.ui_crack().width),
                      static_cast<float>(textures.ui_crack().height)},
            Rectangle{scaled_pos.x, scaled_pos.y, size, size}, Vector2{0, 0}, 0,
            color);
        break;
      }
      break;
    ...
    case 2:
      switch (_geo_index) {
      case 0:
        DrawTexturePro(textures.bathive(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.bathive().width),
                                 static_cast<float>(textures.bathive().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;

      case 1:
        DrawTexturePro(textures.forbid(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.forbid().width),
                                 static_cast<float>(textures.forbid().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;
      ...
      case 2:
        DrawTexturePro(
            textures.wormgrass(),
            Rectangle{0, 0, static_cast<float>(textures.wormgrass().width),
                      static_cast<float>(textures.wormgrass().height)},
            Rectangle{scaled_pos.x, scaled_pos.y, size, size}, Vector2{0, 0}, 0,
            color);
        break;
      ...
      case 3:
        DrawTexturePro(
            textures.waterfall(),
            Rectangle{0, 0, static_cast<float>(textures.waterfall().width),
                      static_cast<float>(textures.waterfall().height)},
            Rectangle{scaled_pos.x, scaled_pos.y, size, size}, Vector2{0, 0}, 0,
            color);
        break;
      }
    ...
    case 3:
      ...
      case 0:
        DrawTexturePro(
            ...
            Rectangle{0, 0, static_cast<float>(textures.entry_loose().width),
                      static_cast<float>(textures.entry_loose().height)},
            Rectangle{scaled_pos.x, scaled_pos.y, size, size}, Vector2{0, 0}, 0,
            color);
        break;

      case 1:
        DrawTexturePro(textures.ui_path(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.ui_path().width),
                                 static_cast<float>(textures.ui_path().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;

      case 2:
        DrawTexturePro(textures.passage(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.passage().width),
                                 static_cast<float>(textures.passage().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;

      case 3:
        DrawTexturePro(textures.den(),
                       Rectangle{0, 0, static_cast<float>(textures.den().width),
                                 static_cast<float>(textures.den().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;

      case 4:
        DrawTexturePro(textures.wack(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.wack().width),
                                 static_cast<float>(textures.wack().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;

      case 5:
        DrawTexturePro(textures.scav(),
                       Rectangle{0, 0,
                                 static_cast<float>(textures.scav().width),
                                 static_cast<float>(textures.scav().height)},
                       Rectangle{scaled_pos.x, scaled_pos.y, size, size},
                       Vector2{0, 0}, 0, color);
        break;
      ...
      case 6:
        DrawTexturePro(
            ...
            Rectangle{0, 0, static_cast<float>(textures.garbageworm().width),
                      static_cast<float>(textures.garbageworm().height)},
            Rectangle{scaled_pos.x, scaled_pos.y, size, size}, Vector2{0, 0}, 0,
            color);
        break;
      }
...
void Geo_Page::windows() noexcept {
  ...
  if (menu_opened) {
    ...
    rlImGuiImageSize(&textures.ui_category_one(), available_width - 20,
                     available_width - 20);

    if (ImGui::GetMouseClickedCount(ImGuiMouseButton_Left) && hovered) {
      _geo_category_index = 0;
    }
    ...
    rlImGuiImageSize(&textures.ui_category_two(), available_width - 20,
                     available_width - 20);
    ...
    rlImGuiImageSize(&textures.ui_category_three(), available_width - 20,
                     available_width - 20);
    ...
    rlImGuiImageSize(&textures.ui_category_four(), available_width - 20,
                     available_width - 20);
      ...
      case 0: {
        {
          ...
          draw_list->AddRectFilled(
              cursor_pos, ImVec2{cursor_pos.x + 34, cursor_pos.y + 34},
              ImGui::ColorConvertFloat4ToU32(ImVec4{1, 1, 1, 1}));

          ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 3);
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 45);

          if (ImGui::Selectable("Solid", _geo_index == 0, 0, selectable_size))
            _geo_index = 0;
        }
        {
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 3);
          ...
          if (ImGui::Selectable("Slope", _geo_index == 1, 0, selectable_size))
            _geo_index = 1;
        }
        {
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 3);
          ...
          if (ImGui::Selectable("Platform", _geo_index == 2, 0,
                                selectable_size))
            _geo_index = 2;
        }
      } break;
      ...
      case 1: {
        {
          ...
          draw_list->AddRectFilled(
              cursor_pos, ImVec2{cursor_pos.x + 6, cursor_pos.y + 34},
              ImGui::ColorConvertFloat4ToU32(ImVec4{1, 1, 1, 1}));

          ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 3);
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 45);

          if (ImGui::Selectable("Vertical Pole", _geo_index == 0, 0,
                                selectable_size))
            _geo_index = 0;
        }
        {
          ...
          draw_list->AddRectFilled(
              cursor_pos, ImVec2{cursor_pos.x + 34, cursor_pos.y + 6},
              ImGui::ColorConvertFloat4ToU32(ImVec4{1, 1, 1, 1}));

          ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 3);
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 45);

          if (ImGui::Selectable("Horizontal Pole", _geo_index == 1, 0,
                                selectable_size))
            _geo_index = 1;
        }
        {
          ...
          if (ImGui::Selectable("Crack", _geo_index == 2, 0, selectable_size))
            _geo_index = 2;
        }
      } break;
      ...
      case 2: {
        {
          ...
          if (ImGui::Selectable("Bathive", _geo_index == 0, 0, selectable_size))
            _geo_index = 0;
        }
        {
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 3);
          ...
          if (ImGui::Selectable("Forbid Fly Chains", _geo_index == 1, 0,
                                selectable_size))
            _geo_index = 1;
        }
        {
          ...
          if (ImGui::Selectable("Worm Grass", _geo_index == 2, 0,
                                selectable_size))
            _geo_index = 2;
        }
        {
          ...
          if (ImGui::Selectable("Waterfall", _geo_index == 3, 0,
                                selectable_size))
            _geo_index = 3;
        }
      } break;
      ...
      case 3: {
        {
          ...
          if (ImGui::Selectable("Shortcut Entry", _geo_index == 0, 0,
                                selectable_size))
            _geo_index = 0;
        }
        {
          ...
          if (ImGui::Selectable("Shortcut Path", _geo_index == 1, 0,
                                selectable_size))
            _geo_index = 1;
        }
        {
          ...
          if (ImGui::Selectable("Passage", _geo_index == 2, 0, selectable_size))
            _geo_index = 2;
        }
        {
          ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 3);
          ...
          if (ImGui::Selectable("Dragon Den", _geo_index == 3, 0,
                                selectable_size))
            _geo_index = 3;
        }
        {
          ...
          if (ImGui::Selectable("Wach-a-mole Hole", _geo_index == 4, 0,
                                selectable_size))
            _geo_index = 4;
        }
        {
          ...
          if (ImGui::Selectable("Scavenger", _geo_index == 5, 0,
                                selectable_size))
            _geo_index = 5;
        }
        {
          ...
          if (ImGui::Selectable("Garbage Worm", _geo_index == 6, 0,
                                selectable_size))
            _geo_index = 6;
        }
      } break;
...
void Geo_Page::order_level_redraw() noexcept { should_redraw = true; }

void Geo_Page::on_level_loaded() noexcept { should_redraw = true; }

void Geo_Page::on_level_unloaded() noexcept { should_redraw = true; }

void Geo_Page::on_level_selected() noexcept { should_redraw = true; }

void Geo_Page::on_page_selected() noexcept { should_redraw = true; }

Geo_Page::Geo_Page(context *ctx)
    : LevelPage(ctx), _hovering_on_window(false), should_redraw1(true),
      should_redraw2(true), should_redraw3(true), should_redraw_feature1(true),
      should_redraw_feature2(true), should_redraw_feature3(true),
      should_redraw(true), _geo_category_index(0), _geo_index(0),
      _edit_mode(EDIT_MODE_PLACE), _is_selecting(false), _erase_all(false),
      _erase_all_features(false), _selection_rect(Rectangle{0, 0, 0, 0}),
      _selection_origin(Vector2{0, 0}) {}
