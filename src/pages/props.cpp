...
void Props_Page::windows() noexcept {
    ...
    if (
        ...
        if (ImGui::BeginListBox("##CategoryTiles", ImGui::GetContentRegionAvail())) {
            const auto &category_tiles = tiles->sorted_tiles().at(_selected_tile_category_index);
    ...
    if (
        ...
        // Prop Categories
        if (ImGui::BeginListBox("##Categories", ImGui::GetContentRegionAvail()) && !props->categories().empty()) {
            for (size_t c = 0; c < props->categories().size(); c++) {
                ...
                bool selected = ImGui::Selectable(category.name.c_str(), _selected_prop_category_index == c);
                if (selected) {
        ...
        // Props
        if (ImGui::BeginListBox("##CategoryProps", ImGui::GetContentRegionAvail()) && 
            !props->categories().empty() && 
            !props->sorted_props().empty() && 
            !props->sorted_props().at(_selected_prop_category_index).empty()
        ) {
            
            const auto &category_props = props->sorted_props().at(_selected_prop_category_index);

            for (size_t t = 0; t < category_props.size(); t++) {
                auto *propdef = category_props[t];

                bool selected = ImGui::Selectable(propdef->name.c_str(), t == _selected_prop_index);
                if (selected) {
...
Props_Page::Props_Page(context *ctx) : 
    LevelPage(ctx), 
    _selected_tile(nullptr), _hovered_tile(nullptr),
